...
async def extract_node(state: dict):
    ...
    if not is_complete:
        ...
        msg = f"当前合同关键信息不完整，缺少：{missing_str}。请补充这些要素后，再用一句完整的话重新描述您的合同需求。"
        return {
...
async def category_node(state: dict):
    ...
    prompt = (
        "请根据以下已提取的合同要素信息，判断本问题应当主要适用哪一类法律语料，"
        "并从系统给定的 category 枚举值中选择一个最合适的类别；若无法判断，请返回 null。\n"
        f"已提取要素（JSON 格式）：\n{info_json}\n"
        "请严格只输出这一行 category 字符串本身，不要附加任何说明文字。"
    )
    category_llm = get_llm_chain("src/app/shared/prompts/ragcategory.jinja2")
    # 2. 调用 Agent 进行分类
    response = await category_llm.ainvoke({"input": prompt})
...
async def retriver_node(state: dict):
    ...
    try:
        ...
        response = rag_retrieve_tool.invoke(
            {"query": search_query, "category": info.get("category"), "k": 3}
        )
...
async def generator_node(state: dict):
    ...
    try:
        # 使用专门的合同总结提示词构建链
        chain = get_llm_chain("src/app/shared/prompts/contract_generator_prompt.jinja2")
...
async def check_node(state: dict):
    ...
    try:
        # 使用专门的检查提示词构建链
        chain = get_llm_chain("src/app/shared/prompts/contract_check_prompt.jinja2")
        payload = {"contract_info": json.dumps(info, ensure_ascii=False, indent=2)}
...
async def output_node(state: dict):
    ...
    try:
        chain = get_llm_chain("src/app/shared/prompts/output_prompt.jinja2")
        ...
        if check_result.get("is_pass"):
            ...
            text = f"合同说明存在以下问题：{', '.join(issues)}。建议：{suggestion}"
