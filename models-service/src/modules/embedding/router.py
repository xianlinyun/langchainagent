    ...
    for model_id, config in MODEL_CONFIG_REGISTRY.items():
        ...
        try:
            ...
            if config["type"] == "text-embed":
                ...
                models[model_id] = AutoModelForSpeechSeq2Seq.from_pretrained(
                    model_path
                ).to(device)
...
@app.post("/embed")
async def embed(payload: EmbedRequest, _=Security(get_api_key)):
    ...
    if model_name not in models or model_name not in tokenizers:
        raise HTTPException(
            status_code=404, detail=f"模型 {model_name} 未加载或不支持嵌入"
        )
...
async def list_models(_=Security(get_api_key)):
    """方便前端查看当前激活了哪些模型"""
    return {"active_models": list(models.keys()), "device": device}
